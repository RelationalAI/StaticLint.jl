...
# error_msg -> type
const error_msg2type = Dict()

function reset_static_lint_caches()
    empty!(check_cache)
    empty!(error_msgs)
    empty!(error_msg2type)
...
function generic_check(t::ExtendedRule, x::EXPR, template_code::String, error_msg)
    error_msg isa String && get!(error_msgs, template_code, error_msg)
    error_msg2type[error_msg] = typeof(t)
...
check(t::CFunction_Extension, x::EXPR) = generic_check(t, x, "@cfunction(hole_variable, hole_variable_star)", "Macro `@cfunction` should not be used.")
